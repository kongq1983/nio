 ...
 * @since 2020-0630
 */
public class MoreMyProtocolDemo {
...
//        System.out.println(Integer.toHexString(BLANK_MAGIC_CODE));
    ...
    private static void writeNumber(String path) throws Exception {
        int size = 18;
    ...
    private static void write5Char(String path) throws Exception {
        int size = 13;
        ByteBuffer buffer =ByteBuffer.allocate(size);
        buffer.putInt(size); // 4
        buffer.putInt(MESSAGE_MAGIC_CODE); //4
    ...
    private static void write6Char(String path) throws Exception {
        int size = 14;
        ByteBuffer buffer =ByteBuffer.allocate(size);
        buffer.putInt(size); // 4
        buffer.putInt(MESSAGE_MAGIC_CODE); //4
        buffer.put("asdfgh".getBytes());
        buffer.flip();
