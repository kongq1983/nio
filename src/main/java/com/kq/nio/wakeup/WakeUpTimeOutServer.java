 ...
 * @since 2020-0630
 */
public class WakeUpTimeOutServer {

    private Selector selector;
    private ServerSocketChannel serverSocketChannel;

    public WakeUpTimeOutServer() throws Exception {
        this.init();
    }
    ...
    public void start() throws Exception {
        while(true) {
            ...
            int selectSize = this.selector.select(3000);
            //wakeup调用情况下  由于那边sleep=5s  所以每隔3s、2s循环打印   wakeup线程结束后，每隔3s打印
...
//        channel.register(this.selector, SelectionKey.OP_WRITE);//为通道注册读事件
    ...
    public static void main(String[] args) throws Exception {
        WakeUpTimeOutServer wakeUpServer = new WakeUpTimeOutServer();
